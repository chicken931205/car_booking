...
class CPBSWidget extends WP_Widget 
{
	/**************************************************************************/
	...
	function widget($argument,$instance)
	{
		...
		$Template=new CPBSTemplate($data,PLUGIN_CPBS_TEMPLATE_PATH.$argument['_data']['file']);
	...
	function form($instance)
	{
		...
		if(is_array($instance['_data']['field']))
		{
			...
			{
				CPBSHelper::removeUIndex($instance,$value);
				CPBSHelper::removeUIndex($data['option'][$value],'id','name','value');
		...
		$Template=new CPBSTemplate($data,PLUGIN_CPBS_TEMPLATE_PATH.'admin/'.$instance['_data']['file']);
