...
class CPBSWooCommerce
{
	...
	function sendBooking($bookingId,$bookingForm,$data)
	{
		...
		update_post_meta($order->get_id(),PLUGIN_CPBS_CONTEXT.'_booking_id',$bookingId);
		
		CPBSPostMeta::updatePostMeta($bookingId,'woocommerce_booking_id',$order->get_id());
		...
		if(isset($billing['detail']))
		{
			...
			foreach($billing['detail'] as $detail)
			{
				...
				(
					...
					(
						...
						'meta'=>array
						(
							'cpbs_price_gross'=>$detail['value_gross'],
							'cpbs_tax_value'=>$detail['tax_value'],
			...
			foreach($orderItem as $item)
			{
				...
				$priceGross=get_post_meta($item->get_product_id(),'cpbs_price_gross',true);
				$taxValue=get_post_meta($item->get_product_id(),'cpbs_tax_value',true);
				$taxAmount=round($priceGross-$priceNet,2);
				$taxLabel=sprintf(esc_html__('Tax %01.2f','car-park-booking-system'),$taxValue);
	...
	function locateTemplate($template,$templateName,$templatePath) 
	{
		...
		$pluginPath=PLUGIN_CPBS_PATH.'woocommerce/';
	...
	function addMetaBox()
	{
		...
		if($post->post_type=='shop_order')
		{
			...
			{
				add_meta_box(PLUGIN_CPBS_CONTEXT.'_meta_box_woocommerce_product',esc_html__('Booking','car-park-booking-system'),array($this,'addMetaBoxWooCommerceBooking'),'shop_order','side','low');		
			}
		}
	}
	...
	function addMetaBoxWooCommerceBooking()
	{
		...
		'
			<div>
				<div>'.esc_html__('This order has corresponding booking from "Car Park Booking System" plugin. Click on button below to see its details in new window.','car-park-booking-system').'</div>
				<br/>
				<a class="button button-primary" href="'.esc_url(get_edit_post_link($meta['booking_id'])).'" target="_blank">'.esc_html__('Open booking','car-park-booking-system').'</a>
			</div>
		';
	}
	...
	function changeStatus($orderId=-1,$bookingId=-1,&$emailSend=false)
	{
		...
		$bookingStatusSynchronizationId=(int)CPBSOption::getOption('booking_status_synchronization');
		...
		if((int)$orderId!==-1)
		{
			$orderMeta=CPBSPostMeta::getPostMeta($orderId);		
		...
		if((int)$bookingStatusSynchronizationId===2)
		{
			if($bookingId!=-1)
			{
				...
				{	
					...
					CPBSPostMeta::updatePostMeta($bookingId,'booking_status_id',$status);
	...
	function createOrderEmailMessageBody($order,$sent_to_admin=false)
	{
		...
		$meta=CPBSPostMeta::getPostMeta($order->get_id());
		...
		$Template=new CPBSTemplate($data,PLUGIN_CPBS_TEMPLATE_PATH.'email_booking.php');
	...
	function addEmailRecipient($recipient,$order)
	{
		...
		if(count($location)!=1) return($recipient);
		...
		if((int)$location[$locationId]['meta']['booking_new_woocommerce_email_notification']===1)
			$t=preg_split('/;/',$location[$locationId]['meta']['booking_new_recipient_email_address']);
			
		if(is_array($t))
		{
			foreach($t as $value)
				array_push($emailAddress,$value);
		}
	...
	function addEmailAttachment($attachments,$status)
	{
		if($status=='new_order')
		{
			$attachments[]=wp_get_attachment_url(CPBSOption::getOption('attachment_woocommerce_email'));
		}
