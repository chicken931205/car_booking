...
class CPBSGeoLocation
{
	...
	function __construct()
	{
		...
		(
			1=>array
			(
				'name'=>esc_html__('KeyCDN [keycdn.com]','car-park-booking-system'),
				'api_url_address'=>'https://tools.keycdn.com/geo.json?host={CLIENT_IP}'
			),
			2=>array
			(
				'name'=>esc_html__('IP-API [ip-api.com]','car-park-booking-system'),
				'api_url_address'=>'http://ip-api.com/json/{CLIENT_IP}'
			),
			3=>array
			(
				'name'=>esc_html__('ipstack [ipstack.com]','car-park-booking-system'),
	...
	{
		if(!array_key_exists(CPBSOption::getOption('geolocation_server_id'),$this->getServer())) return(false);
		
		if(!ini_get('allow_url_fopen')) return(false);
	 
		$sessionStart=true;
		$sessionIndex=PLUGIN_CPBS_CONTEXT.'_customer_geolocation_document';
		...
		else
		{
			...
			$addressURL=$this->server[CPBSOption::getOption('geolocation_server_id')]['api_url_address'];

			if(CPBSOption::getOption('geolocation_server_id')==3)
				$addressURL=preg_replace(array('/{CLIENT_IP}/','/{API_KEY}/'),array($addressIP,CPBSOption::getOption('geolocation_server_id_3_api_key')),$addressURL);
			...
			if(($document=@file_get_contents($addressURL,false,$context))===false) 
				return(false);
		...
		switch(CPBSOption::getOption('geolocation_server_id'))
		{
			case 1:
